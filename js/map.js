...
require([
    ...
    // THIS SOURCE LOCKS SEARCH WIDGET - REMOVED FOR TESTING, NEED TO DEBUG
    // searchWidget.sources.push({
    //     featureLayer: new FeatureLayer({
    ...
    const roadways = new FeatureLayer({
        ...
        definitionExpression: "RTE_PRFX = 'IH' AND RDBD_TYPE = 'Single Roadbed'",
        legendEnabled: false,
        visible: true,
        opacity: 0.0,
        popupEnabled: true,
    ...
    // CLEAR ALL OVERLAYS AND ONLY TOGGLE ICON IF IT'S ON (need to loop using layer index)
    ...
    function scaleDependentQueries() {
        if (mapView.zoom<8) {
           roadways.definitionExpression = "RTE_PRFX = 'IH' AND RDBD_TYPE = 'Single Roadbed'";
        }
        if (mapView.zoom>7&&mapView.zoom<9) {
           roadways.definitionExpression = "RTE_PRFX IN ('IH', 'US') AND RDBD_TYPE = 'Single Roadbed'";
        }
        if (mapView.zoom>8&&mapView.zoom<10) {
           roadways.definitionExpression = "RTE_PRFX IN ('IH', 'US', 'SH', 'SL', 'FM', 'RM') AND RDBD_TYPE = 'Single Roadbed'";
        }
        if (mapView.zoom>9&&mapView.zoom<13) {
           roadways.definitionExpression = "RTE_PRFX NOT IN ('CS', 'PR') AND RDBD_TYPE = 'Single Roadbed'";
        }
        if (mapView.zoom>12) {
           roadways.definitionExpression = "";
        }
        if (controlSection.visible) {
            if (mapView.zoom<9) {
    ...
    // JUMP TO GOOGLE AND BING MAPS (need to consolidate code here)
    // https://gis.stackexchange.com/questions/222652/getting-current-zoom-level-in-arcgis-api-for-javascript-4/223152
    $('#google').click(function() {
        const mapCenter = mapView.get('center');
    ...
    $('#bing').click(function() {
        ...
        const lon = mapCenter.longitude;
        const zoom = mapView.zoom;
        window.open("https://www.bing.com/mapspreview?cp="+lat+"~"+lon+"&lvl="+zoom+"&style=r");
    });
